...
async def create_new_thread(thread: model_type.AssistantThread):
        ...
        save_created_thread(thread, created_thread.id)
        return {"thread": created_thread}


async def get_all_threads(assistant_id: str):
        result = fetch_threads_by_assistant_id(assistant_id)
        return result
